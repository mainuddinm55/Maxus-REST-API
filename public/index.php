...
$app->post('/createshareduser', function (Request $request, Response $response) {
    ...
    if ($result == DATA_INSERTED) {
        ...
        return $response->withHeader('Content-type', 'application/json')
            ->withStatus(200);
    } else if ($result == MOBILE_DUPLICATE) {
        $message = array();
        $message['error'] = true;
        ...
        return $response->withHeader('Content-type', 'application/json')
            ->withStatus(200);
    }else if ($result == USERNAME_DUPLICATE) {
        $message = array();
        $message['error'] = true;
        $message['message'] = 'Username already exists taken';
        $response->write(json_encode($message));
        return $response->withHeader('Content-type', 'application/json')
            ->withStatus(200);
    }else if ($result == EMAIL_DUPLICATE) {
        $message = array();
        $message['error'] = true;
...
$app->put('/seennotification/{id}', function (Request $request, Response $response, array $args) {
    $id = $args['id'];
    $db = new DbOperations();
    if ($db->seenNotification($id)) {
        $message = array();
        $message['error'] = false;
        $message['message'] = 'Seen Notification';
        $response->write(json_encode($message));
    } else {
        $message = array();
        $message['error'] = true;
        $message['message'] = 'Seen Notification Failed';
        $response->write(json_encode($message));
    }
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
...
// Get Bet Result
$app->get('/getbetresult/{user_id}/{bet_id}', function (Request $request, Response $response, array $args) {
    $user_id = $args['user_id'];
    $bet_id = $args['bet_id'];
    $db = new DbOperations();

    $bet_result = $db->getBetResult($bet_id, $user_id);
    $response->write(json_encode($bet_result));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
$app->put('/appreovedtransaction', function (Request $request, Response $response) {
    ...
    $db = new DbOperations();
    $result = $db->withdrawUserBalanceByUsername($amount, $to_username);
    if ($result) {
        $result = $db->depositUserBalanceByUsername($amount, $from_username);
        if ($result) {
            $message = array();
            $message['error'] = false;
...
$app->get('/optionbyid/{id}', function (Request $request, Response $response, array $args) {
    $id = $args['id'];
    $db = new DbOperations();
    $option = $db->getOptionById($id);
    $response->write(json_encode($option));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
$app->post('/addcommission', function (Request $request, Response $response) {
    ...
    if ($result == DATA_INSERTED) {
        ...
        $db->depositUserBalanceByUsername($amount, $username);
        $response->write(json_encode($message));
        return $response->withHeader('Content-type', 'application/json')
            ->withStatus(201);
    } else if ($result == DATA_INSERTED_FAILED) {
...
$app->get('/commissionbyid/{id}', function (Request $request, Response $response, array $args) {
    ...
    $response->write(json_encode($commission));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});

$app->get('/commissionbyusername/{username}', function (Request $request, Response $response, array $args) {
    $username = $args['username'];
    $db = new DbOperations();
    $commission = $db->getCommissionByUsername($username);
    $response->write(json_encode($commission));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
$app->get('/usertotalbonus/{username}', function (Request $request, Response $response, array $args) {
    $username = $args['username'];
    $db = new DbOperations();
    $bonus = $db->getTotalBonus($username);
    $response->write(json_encode($bonus));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
$app->post('/userbet', function (Request $request, Response $response) {
    ...
    if ($result == DATA_INSERTED) {
        if ($bet_mode_id==2) {
            $db->withdrawUserBalance($bet_amount, $user_id);
            $db->depositUserBalance($bet_amount, 1);
        }
        $message = array();
        $message['error'] = false;
...
$app->get('/userbetshistory/{id}', function (Request $request, Response $response, array $args) {
    $id = $args['id'];
    $db = new DbOperations();
    $bet_history = $db->getUserBetHistory($id);
    $response->write(json_encode($bet_history));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});

$app->get('/gettotalbetotioncalculation/{bet_id}', function (Request $request, Response $response, array $args) {
    $bet_id = $args['bet_id'];
    $db = new DbOperations();

    $calculation = $db->getTotalCalculationByBetId($bet_id);
    $response->write(json_encode($calculation));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
$app->get('/allusersbets',function(Request $request, Response $response){
    $db = new DbOperations();
    $userbets = $db->getUsersBets();
    $response->write(json_encode($userbets));
    return $response->withHeader('Content-type','application/json')
    ->withStatus(200);
});
...
// Get All Club users
$app->get('/getallclubuser/{club_id}', function (Request $request, Response $response, array $args) {
    $club_id = $args['club_id'];
    $db = new DbOperations();
    $users = $db->getAllUsersByClubId($club_id);
    $response->write(json_encode($users));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
// Get All Club Agent
$app->get('/getallclubagent/{club_id}', function (Request $request, Response $response, array $args) {
    $club_id = $args['club_id'];
    $db = new DbOperations();
    $users = $db->getAllClubAgent($club_id);
    $response->write(json_encode($users));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
$app->get('/getallagentuser/{agentid}', function (Request $request, Response $response, array $args) {
    $agent_id = $args['agentid'];
    $db = new DbOperations();

    $users = $db->getAllAgentUser($agent_id);
    $response->write(json_encode($users));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});
...
$app->post('/createsecuritypin', function (Request $request, Response $response) {
    ...
    if ($result == DATA_INSERTED) {
        ...
        $message = array();
        $message['error'] = true;
        $message['message'] = 'Already exists this pin';
        $response->write(json_encode($message));
        return $response->withHeader('Content-type', 'application/json')
            ->withStatus(201);
    }
...
$app->get('/allsecuritypin', function (Request $request, Response $response) {
    $db = new DbOperations();
    $pin = $db->getAllSecuritypin();
    $response->write(json_encode($pin));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});

$app->get('/ranks', function (Request $request, Response $response) {
    $db = new DbOperations();
    $ranks = $db->getAllRank();
    $response->write(json_encode($ranks));
    return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
});

///Get User Security pin
    $app->get('/alluserpin/{id}', function (Request $request, Response $response, array $args) {
        $id = $args['id'];
        $db = new DbOperations();
        $pin = $db->getAllUserpin($id);
        $response->write(json_encode($pin));
        return $response->withHeader('Content-type', 'application/json')
        ->withStatus(200);
    });
