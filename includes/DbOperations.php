...
class DbOperations
{
    ...
    {
        $stmt = $this->con->prepare("SELECT user_id FROM profit_shared_user WHERE email = ?");
        $stmt->bind_param("s",$email);
        $stmt->execute();
        $stmt->store_result();
        if(!$stmt->num_rows>0){
            $stmt = $this->con->prepare("SELECT user_id FROM profit_shared_user WHERE username = ?");
            $stmt->bind_param("s",$username);
            $stmt->execute();
            $stmt->store_result();
            if(!$stmt->num_rows>0){
                $stmt = $this->con->prepare("SELECT user_id FROM profit_shared_user WHERE mobile = ?");
    ...
    public function depositUserBalance($depositAmount, $user_id)
    {
        $stmt = $this->con->prepare("UPDATE users SET total_balance = total_balance+? WHERE user_id = ?");
        $stmt->bind_param("di", $depositAmount, $user_id);
        return $stmt->execute();
    }
    public function depositUserBalanceByUsername($depositAmount, $username)
    {
        $stmt = $this->con->prepare("UPDATE users SET total_balance = total_balance+? WHERE username = ?");
        $stmt->bind_param("ds", $depositAmount, $username);
        return $stmt->execute();
    }

    public function withdrawUserBalance($withdrawAmount, $user_id)
    {
        $stmt = $this->con->prepare("UPDATE users SET total_balance = total_balance - ? WHERE user_id = ?");
...
/* 
    ...
    {
        $stmt = $this->con->prepare("UPDATE users SET total_balance = total_balance+? WHERE username = ?");
        $stmt->bind_param("di", $depositAmount, $username);
        return $stmt->execute();
    }
 */
    public function withdrawUserBalanceByUsername($withdrawAmount, $username)
    {
        $stmt = $this->con->prepare("UPDATE users SET total_balance = total_balance - ? WHERE username = ?");
        $stmt->bind_param("di", $withdrawAmount, $username);
        return $stmt->execute();
    }

    public function updateUserBalanceById($amount, $user_id)
    {
        $stmt = $this->con->prepare("UPDATE users SET total_balance = ? WHERE user_id = ?");
    ...
    public function getBetResult($bet_id, $user_id){
        $stmt = $this->con->prepare("SELECT matches.team1, matches.team2, bet.question, bet.right_answer, user_bet.bet_option_id, user_bet.bet_amount,user_bet.bet_return_amount, user_bet.result FROM matches, bet, user_bet WHERE matches.id = bet.match_id AND bet.bet_id = user_bet.bet_id AND user_bet.user_id = ? AND user_bet.bet_id = ?");
        $stmt->bind_param("ii", $user_id,$bet_id);
        $stmt->execute();
        $stmt->bind_result($team1,$team2,$question,$right_answer,$bet_option_id,$bet_amount,$bet_return_amount,$result);
        $bet_result = array();
        while ($stmt->fetch()) {
            $bet_result['match'] = $team1." vs ".$team2;
            $bet_result['question'] = $question;
            $bet_result['right_ans'] = $right_answer;
            $bet_result['bet_option_id'] = $bet_option_id;
            $bet_result['bet_amount'] = $bet_amount;
            $bet_result['bet_return_amount'] = $bet_return_amount;
    ...
    {
        $stmt = $this->con->prepare("SELECT * FROM notification WHERE username = ?");
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $stmt->bind_result($id, $username, $body, $type, $type_id, $notice_date, $seen);
        $notifications = array();
        while ($stmt->fetch()) {
            $notification = array();
            $notification['id'] = $id;
    ...
    public function seenNotification($id){
        $stmt = $this->con->prepare("UPDATE notification SET seen = 1 WHERE id =?");
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) {
            return true;
        }else {
    ...
    public function getOptionById($id){
        $stmt = $this->con->prepare("SELECT bet_rate.options from bet_rate where bet_rate.id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
    ...
    public function getTotalBonus($username){
        $stmt = $this->con->prepare("SELECT format(SUM(commission.amount),4) as bonus FROM commission WHERE commission.username = ?");
        $stmt->bind_param("s", $username);
        $stmt->execute();
    ...
    public function getCommissionByUsername($username)
    {
        $stmt = $this->con->prepare("SELECT comm_id,comm_rate, amount, username, from_user_id, bet_id, comm_date,purpose FROM commission WHERE username = ?");
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $stmt->bind_result($comm_id, $comm_rate, $amount, $username, $from_user_id, $bet_id, $comm_date, $purpose);
        $commissions = array();
    ...
    public function getTotalCalculationByBetId($bet_id)
    {
        $stmt = $this->con->prepare("SELECT SUM(bet_amount), SUM(bet_return_amount), bet_option_id FROM user_bet WHERE bet_id = ? GROUP BY bet_option_id");
        $stmt->bind_param("i", $bet_id);
        $stmt->execute();
        $stmt->bind_result($bet_amount, $bet_return_amount, $bet_option_id);
        // $stmt->fetch();
        $calculations = array();
        while ($stmt->fetch()) {
            $calculation = array();
            $calculation['bet_id'] = $bet_id;
            $calculation['bet_option_id'] = $bet_option_id;
            $calculation['bet_amount'] = $bet_amount;
            $calculation['bet_return_amount'] = $bet_return_amount;
            array_push($calculations, $calculation);
        }
        return $calculations;
    }
    public function getUserBetHistory($user_id){
        $stmt = $this->con->prepare("SELECT user_bet.bet_date, bet.question, user_bet.bet_rate, user_bet.bet_amount, user_bet.bet_return_amount, user_bet.result FROM user_bet, bet WHERE user_bet.bet_id = bet.bet_id AND user_bet.user_id =?");
        $stmt->bind_param("i", $user_id);
        $stmt->execute();
        $stmt->bind_result($bet_date, $question,$bet_rate,$bet_amount,$bet_return_amount,$result);
        $bets = array();
        while ($stmt->fetch()){
            $bet = array();
            $bet['bet_date'] = $bet_date;
            $bet['question'] = $question;
            $bet['bet_rate'] = $bet_rate;
            $bet['bet_amount'] = $bet_amount;
            $bet['bet_return_amount'] = $bet_return_amount;
            $bet['result'] = $result;
            array_push($bets, $bet);
        }
        return $bets;
    ...
    //Get All user betting
    public function getUsersBets()
    {
        $stmt = $this->con->prepare("SELECT ub.user_id, ub.bet_id, ub.bet_option_id, ub.bet_rate, ub.bet_amount, ub.bet_return_amount, ub.bet_mode_id, ub.bet_date, ub.result, u.username, u.mobile, b.question FROM user_bet ub, users u, bet b WHERE ub.user_id = u.user_id AND ub.bet_id = b.bet_id");
        $stmt->execute();
        $stmt->bind_result($user_id, $bet_id, $bet_option_id, $bet_rate, $bet_amount, $bet_return_amount, $bet_mode_id, $bet_date, $result, $username, $mobile, $question);
        $user_bets = array();
        while ($stmt->fetch()) {
    ...
    public function getUserByEmail($email)
    {
        ...
        $stmt->execute();
        $stmt->bind_result($club_id, $name, $username, $email, $mobile, $district, $upazilla, $up, $total_balance, $status, $type_id);
    ...
    public function increaseRefCount($reference)
    {
        $stmt = $this->con->prepare("UPDATE users SET ref_count = ref_count + 1 WHERE username = ?");
    ...
    {
        $stmt = $this->con->prepare("SELECT agent_id FROM users WHERE username = ? AND type_id IN (4,5,6)");
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $stmt->bind_result($agent_id);
        $stmt->fetch();
        if ($agent_id==null) {
            $stmt = $this->con->prepare("SELECT user_id FROM users WHERE username = ? AND type_id IN (3)");
            $stmt->bind_param("s", $username);
            $stmt->execute();
    ...
    public function getAllAgentUser($agent_id)
    {
        $stmt = $this->con->prepare("SELECT user_id, name, username, email, mobile, reference, agent_id, district, upazilla, up, type_id, total_balance, status, rank_id FROM users WHERE agent_id = ?");
        $stmt->bind_param("i", $agent_id);
        $stmt->execute();
        $stmt->bind_result($user_id, $name, $username, $email, $mobile, $reference, $agent_id, $district, $upazilla, $up, $type_id, $total_balance, $status, $rank_id);
    ...
    public function getAllUsersByClubId($id){
        $stmt = $this->con->prepare("SELECT user_id, name, username, email, mobile, club_id, reference, agent_id, district,upazilla,up,total_balance, status,rank_id,type_id FROM users WHERE users.agent_id IN (SELECT users.user_id FROM users WHERE users.club_id = ?)");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->bind_result($user_id, $name, $username, $email, $mobile, $club_id, $reference, $agent_id, $district, $upazilla, $up, $total_balance, $status, $rank_id, $type_id);
        $users = array();
    ...
    //Get All club agent
    public function getAllClubAgent($club_id){
        $stmt = $this->con->prepare("SELECT user_id, name, username, email, mobile, club_id, reference, agent_id, district,upazilla,up,total_balance, status,rank_id,type_id FROM users WHERE club_id = ? AND type_id = 3");
        $stmt->bind_param("i", $club_id);
        $stmt->execute();
        $stmt->bind_result($user_id, $name, $username, $email, $mobile, $club_id, $reference, $agent_id, $district, $upazilla, $up, $total_balance, $status, $rank_id, $type_id);
        $users = array();
    ...
    //Get All Agent Users
    public function getAllUsersByAgentId($id){
        $stmt = $this->con->prepare("SELECT user_id, name, username, email, mobile, club_id, reference, agent_id, district,upazilla,up,total_balance, status,rank_id,type_id FROM users WHERE users.agent_id =? AND type_id IN (4,5,6)");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->bind_result($user_id, $name, $username, $email, $mobile, $club_id, $reference, $agent_id, $district, $upazilla, $up, $total_balance, $status, $rank_id, $type_id);
        $users = array();
    ...
    public function getUsernameById($id){
        $stmt = $this->con->prepare("SELECT username from users where user_id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
    ...
    {
        $stmt = $this->con->prepare("SELECT pin FROM security_pin WHERE pin = ?");
    ...
    {
        $stmt = $this->con->prepare("SELECT id, pin, user_type_id, used, validity, owner_id FROM security_pin;");
        $stmt->execute();
        $stmt->bind_result($id, $pin, $user_type_id, $used, $validity, $owner_id);
        ...
        while ($stmt->fetch()) {
            ...
            $spin['id'] = $id;
            $spin['pin'] = $pin;
            $spin['user_type_id'] = $user_type_id;
            $spin['used'] = $used;
            $spin['validity'] = $validity;
            $spin['owner_id'] = $owner_id;
            array_push($pins, $spin);
        }
        return $pins;
    }
//Get User Security Pin
    public function getAllUserpin($id)
    {
        $stmt = $this->con->prepare("SELECT id, pin, user_type_id, used, validity, owner_id FROM security_pin WHERE owner_id = ?;");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->bind_result($id, $pin, $user_type_id, $used, $validity, $owner_id);
    ...
    public function getAllRank() {
        $stmt = $this->con->prepare("SELECT * FROM rank ORDER BY rank_id");
        $stmt->execute();
        $stmt->bind_result($rank_id, $rank_name, $assert_need,$bonus);
        $ranks = array();
        while ($stmt->fetch()) {
            $rank = array();
            $rank['rank_id'] = $rank_id;
            $rank['rank_name'] = $rank_name;
            $rank['assert_need'] = $assert_need;
            $rank['bonus'] = $bonus;
